...
class NMFWriter:
    ...
    def _pack_anim_into(self, inner: bytearray, item: Dict[str, Any]) -> None:
        ...
        self._i_push_word(inner, 'ANIM')
        self._i_push_int(inner, item['unknown'])
...
def main():
    if len(sys.argv) < 2:
        print("Usage: python pack_nmf.py <path_to_model_json> [output_path.nmf]")
        sys.exit(1)
    ...
    # Имя выходного файла: либо второй аргумент, либо то же имя с .nmf
    out_path = Path(sys.argv[2]) if len(sys.argv) >= 3 else in_path.with_suffix(".nmf")

    # 1) читаем JSON → dict/list (model_file)
    try:
    ...
    # 2) упаковываем через NMFWriter
    try:
        writer = NMFWriter()
        binary_blob = writer.pack(model_file)  # ожидается bytes
        if not isinstance(binary_blob, (bytes, bytearray)):
            print("Error: NMFWriter.pack() must return bytes/bytearray.")
            sys.exit(1)
    except Exception as e:
        print(f"Error in NMFWriter.pack(): {e}")
        sys.exit(1)
