...
This repository contains a collection of Python scripts designed to extract, convert, fix, and repack assets for the game **Der Clou! 2** (also known as *The Sting!* or *Ва-Банк!*).
...
* **Python 3.6+**
* **Dependencies**: Most scripts use the standard library.
* `nmf_to_fbx.py` requires `numpy`.
* Install via: `pip install numpy`

---

## 1. World Management (WLD Files)

Tools for handling the main game resource archives (`.wld`).

### WLD Unpacker

Extracts all contents of a `.wld` file, including textures (converted to DDS), models (extracted as NMF), and hierarchy data (JSON).

* **Script**: `wld_unpacker.py`
* **Usage**:
```bash
python bin/wld_unpacker.py <path_to_file.wld>

```


* **Output**: Creates a folder named `<filename>_unpack` containing organized assets.

### WLD Packer

Repacks an unpacked directory back into a binary `.wld` file usable by the game.

* **Script**: `wld_packer.py`
* **Usage**:
```bash
python bin/wld_packer.py <path_to_unpack_folder>

```

### Texture Extractor

A standalone tool to extract textures from a WLD file without unpacking the whole archive. Supports extraction as `tif`, `png`, or `dds`, and handles texture atlases.

* **Script**: `extract_textures.py`
* **Usage**:
```bash
python bin/extract_textures.py <input.wld> [options]

```

* **Options**:
* `-f`, `--format`: Output format (`tif`, `png`, `dds`, `all`). Default: `tif`.
* `-m`, `--mode`: Extraction mode (`atlas`, `extract`, `both`). Default: `both`.
* `-l`, `--list`: List internal folder paths instead of extracting.

---

## 2. Model Conversion (NMF Files)

Tools for converting the internal 3D format (`.nmf`) to and from modern formats.

### NMF to FBX (Binary)

Converts a model directly to a binary FBX file (7.5 format), including geometry, skeletal hierarchy, and animations. Compatible with Blender, Maya, and Unity.

* **Script**: `nmf_to_fbx.py`
* **Usage**:
```bash
python bin/nmf_to_fbx.py <input.nmf> <output.fbx>

```

### NMF to Maya ASCII

Converts a model to an editable Autodesk Maya (`.ma`) file.

* **Script**: `nmf_to_maya.py`
* **Usage**:
```bash
python bin/nmf_to_maya.py <input.nmf> <output.ma>

```

### NMF to JSON / JSON to NMF

Converts binary model data to human-readable JSON for analysis or manual editing, and packs it back.

* **To JSON**:
```bash
python bin/nmf_to_json.py <input.nmf> <output.json>

```

* **To NMF**:
```bash
python bin/json_to_nmf.py <input.json> <output.nmf>

```

### Encoding/Name Fixer

Fixes character encoding issues (e.g., special characters like `ß`, `ä`, `ö`) inside internal file paths and material names to prevent crashes or loading errors.

* **Script**: `wld_fixer_names.py`
* **Usage**:
```bash
python bin/wld_fixer_names.py <input.wld> <output.wld>

```

### Dialog Decompressor

Decompresses and decrypts the game's text dialog files (`Dialogs_*.txt`). It handles the custom bitmask compression and removes debug padding.

* **Script**: `decompress_dialogs.py`
* **Usage**:
```bash
python bin/decompress_dialogs.py <input_file.txt> <output_file.txt>

```

### Jewellery Shadow Extractor

Specific utility to find and extract shadow data for objects located in the "Jewellery" folder hierarchy of a world file.

* **Script**: `extract_jewellery_shadows.py`
* **Usage**:
```bash
python bin/extract_jewellery_shadows.py <path_to.wld> [--format json|bin]

```

### NMF to FBX JSON (Debug)

Exports the intermediate structure used for FBX generation to a JSON file. Useful for debugging the FBX writer logic. Requires the Blender FBX plugin to convert this JSON to binary format.

* **Script**: `nmf_to_fbx_json.py`
* **Usage**:
```bash
python bin/nmf_to_fbx_json.py <input.nmf> <output_fbx.json>
