...
def pack(filepath)
  ...
  # filepath = "#{File.dirname(filepath)}/#{}_unpack"
  ...
  filepath = File.join(filepath, 'pack')
  unless File.directory?(filepath)
    puts "#{filepath} is not a directory."
    exit 1
  end
  ...
  file = File.open(File.join(basefilepath, filename), 'wb')
  file.write('WRLD')
  file.write(zero)
  ...
  textures_info = YAML.load_file(File.join(filepath, 'texture_pages.yml'))
  textures_info.each do |info|
    data_to_write = []

    texture_file = File.binread(File.join(File.join(filepath, 'texture_pages'), "#{info['index']}.dds"))
  ...
  wld_items.each do |key, _value|
    ...
    item_file = File.open(File.join(filepath, key.downcase.to_s), 'rb')
...
filepaths = Dir.glob(File.join(script_location, '*_unpack'))
if filepaths.nil?
